...
export class CqlProject extends EventEmitter {
  ...
  private configureTestFolderWatcher() {
    this.testFolderWatcher!.onDidCreate(() => this.refresh());
    this.testFolderWatcher!.onDidDelete(() => this.refresh());
  ...
  private async loadResults(): Promise<void> {
    ...
    try {
      let entries: string[];
      try {
        entries = await fs.promises.readdir(this.resultFolder);
      } catch {
        return;
      }
      for (const file of entries) {
        ...
        const lib = this.findLibraryByName(name);
        if (lib) {
          lib.setResult(Uri.file(path.join(this.resultFolder, file)));
          matched++;
        }
      }
    } catch (e) {
   ...
   * @param name base library file name only, no extension or directory needed
   */
  public async createLibrary(name: string): Promise<void> {
    const fullPath = path.join(this.libraryFolder, `${name}.${CqlLibrary.FILE_EXT}`);
    try {
      await fs.promises.writeFile(fullPath, '');
      logger.debug(`library file ${fullPath} created successfully.`);
    } catch (error) {
      logger.error(`unable to create library ${fullPath}`, error);
    }
  }
