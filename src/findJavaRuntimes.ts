...
async function fromEnv(name: string): Promise<string[]> {
	const ret: string[] = [];
	if (process.env[name]) {
		const javaHome = await verifyJavaHome(
			process.env[name]!,
			JAVAC_FILENAME
		);
		if (javaHome) {
		...
		if (isMac) {
			...
			if (await fse.pathExists(macUtility)) {
				...
				try {
					...
					const absoluteJavaHome =
						"" + buffer.toString().replace(/\n$/, "");
					const verified = await verifyJavaHome(
						absoluteJavaHome,
						JAVAC_FILENAME
					);
					if (verified) {
...
async function fromWindowsRegistry(): Promise<string[]> {
	...
	const promisifyFindJavaHomeInRegKey = (
		reg: Winreg.Registry
	): Promise<string | null> => {
	...
	const keysFound: Winreg.Registry[] = Array.prototype.concat.apply(
		[],
		keysFoundSegments
	);
	if (!keysFound.length) {
	...
	if (isWindows) {
		...
		const possibleLocations: string[] = [
			process.env.ProgramFiles &&
				path.join(process.env.ProgramFiles, "Java"), // Oracle JDK per machine
			process.env.ProgramW6432 &&
				path.join(process.env.ProgramW6432, "Java"), // Oracle JDK per machine
			process.env.ProgramFiles &&
				path.join(process.env.ProgramFiles, "AdoptOpenJDK"), // AdoptOpenJDK per machine
			process.env.ProgramW6432 &&
				path.join(process.env.ProgramW6432, "AdoptOpenJDK"), // AdoptOpenJDK per machine
...
export async function verifyJavaHome(
	...
	const targetJavaFile = await findLinkedFile(
		path.resolve(dir, "bin", javaFilename)
	);
...
async function findLinkedFile(file: string): Promise<string> {
	if (
		...
		!(await fse.lstat(file)).isSymbolicLink()
	) {
		return file;
	}
