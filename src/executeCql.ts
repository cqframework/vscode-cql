...
export async function executeCQLFile(uri: Uri, testCases?: TestCase[]): Promise<void> {
  ...
  var testPaths: TestCase[] = testCases ?? getTestPaths(testPath, libraryName);
...
function getFhirVersion(): string | null {
  try {
    ...
    logger.error('error while getting FHIR version', error);
  }
  return null;
}
