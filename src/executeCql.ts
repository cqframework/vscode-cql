...
export async function executeCQLFile(uri: Uri): Promise<void> {
	...
	const modelMessage =
		modelRootPath && modelRootPath !== ""
			? `Data path: ${modelRootPath}`
			: `No data found at ${testPath}. Evaluation may fail if data is required.`;
	const terminologyMessage =
		terminologyPathActual && terminologyPathActual !== ""
			? `Terminology path: ${terminologyPathActual}`
			: `No terminology found at ${terminologyPath}. Evaluation may fail if terminology is required.`;
	...
	if (modelRootPath && modelRootPath !== "" && fs.existsSync(modelRootPath)) {
		const dirs = fs
			...
			.filter((dirent) =>
				fs.statSync(path.join(modelRootPath, dirent)).isDirectory()
			);
...
function getModelRootPath(parentPath: string, libraryPathName: string): string {
	...
	if (fs.existsSync(parentPath)) {
		...
		const dirs = files.filter((dirent) =>
			fs.statSync(path.join(parentPath, dirent)).isDirectory()
		);
		dirs.forEach((dirent) => {
			if (modelRootPath === "") {
				if (dirent === libraryPathName) {
					...
					modelRootPath = getModelRootPath(
						path.join(parentPath, dirent),
						libraryPathName
					);
				}
			}
