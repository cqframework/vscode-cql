...
export async function executeCQLFile(uri: Uri): Promise<void> {
	...
	let terminologyPath : Uri = Utils.resolvePath(
		projectPath,
		"input",
		"vocabulary",
		"valueset"
	...
	for (var p of testPaths) {
		testCasesArgs.push(...getExecArgs(
			libraryDirectory,
			libraryName,
			p.path,
			terminologyPath, 
	...
	const terminologyMessage =
		fs.existsSync(terminologyPath.fsPath)
...
function getTestPaths(testPath: Uri, libraryName: string): TestCase[] {
	...
	let directories = glob.sync(testPath.fsPath + `/**/${libraryName}`)
