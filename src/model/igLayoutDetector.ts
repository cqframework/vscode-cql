...
/**
 * Directory names under input/tests/ that are never FHIR resource type directories
 * and should be excluded from test case discovery.
 */
export const RESERVED_TEST_SUBDIRS = new Set(['results', '__MACOSX']);
...
export function findTestCasesFolder(
  ...
  for (const entry of typeDirs) {
    if (KNOWN_RESOURCE_TYPES.has(entry.name)) continue;
    if (RESERVED_TEST_SUBDIRS.has(entry.name)) continue; // not a FHIR resource type dir
