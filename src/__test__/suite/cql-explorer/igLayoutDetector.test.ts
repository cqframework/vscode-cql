...
suite('findTestCasesFolder()', () => {
  ...
  test('results/ subdirectory is not treated as a resource type dir', () => {
    // A library-named subdirectory inside results/ must not be mistaken for a test case folder.
    mkdir(path.join(testDir, 'input', 'tests', 'results', 'MyMeasure'));
    const result = findTestCasesFolder(testDir, 'MyMeasure');
    expect(result.folder).to.be.null;
    expect(result.deviations).to.be.empty;
  });

  test('__MACOSX/ subdirectory is not treated as a resource type dir', () => {
    mkdir(path.join(testDir, 'input', 'tests', '__MACOSX', 'MyMeasure'));
    const result = findTestCasesFolder(testDir, 'MyMeasure');
    expect(result.folder).to.be.null;
    expect(result.deviations).to.be.empty;
  });
