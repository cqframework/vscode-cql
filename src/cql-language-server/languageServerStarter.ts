...
export function awaitServerConnection(port: string): Thenable<StreamInfo> {
  ...
  return new Promise((res, rej) => {
    const server = net.createServer(stream => {
      server.close();
      log.info('CQL LS connection established on port ' + addr);
      res({ reader: stream, writer: stream });
    });
    server.on('error', rej);
    server.listen(addr, () => {
      server.removeListener('error', rej);
      log.info('Awaiting CQL LS connection on port ' + addr);
    });
    return server;
  });
}
...
export function prepareExecutable(
  ...
  log.info(`Starting CQL server with: ${executable.command} ${executable.args.join(' ')}`);
