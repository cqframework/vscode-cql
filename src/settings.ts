...
let oldConfig: WorkspaceConfiguration = getCqlConfiguration();
...
export function onConfigurationChange() {
	return workspace.onDidChangeConfiguration(params => {
		...
		const newConfig = getCqlConfiguration();
...
function excludeProjectSettingsFilesForWorkspace(workspaceUri: Uri) {
	const excudedConfig = getCqlConfiguration().get(EXCLUDE_FILE_CONFIG);
	if (excudedConfig) {
		...
		if (needUpdate) {
			...
			window.showInformationMessage('Do you want to exclude the VS Code Java project settings files (.classpath, .project. .settings, .factorypath) from the file explorer?', ...items).then((result) => {
				if (result === changeItem.global) {
					...
					const storeInWorkspace = getCqlConfiguration().inspect(EXCLUDE_FILE_CONFIG).workspaceValue;
					getCqlConfiguration().update(EXCLUDE_FILE_CONFIG, false, storeInWorkspace?ConfigurationTarget.Workspace: ConfigurationTarget.Global);
				}
			});
		}
	}
