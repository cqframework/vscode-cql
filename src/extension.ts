	...
	return requirements.resolveRequirements(context).catch(error => {
		...
		return new Promise<void>(async (resolve) => {
			...
			context.subscriptions.push(commands.registerCommand(Commands.EXECUTE_WORKSPACE_COMMAND, (command, ...rest) => {
				...
				if (token) {
					return cqlLanguageClient.getClient().sendRequest(ExecuteCommandRequest.type, params, token);
				} else {
...
async function startServer(context: ExtensionContext, requirements: requirements.RequirementsData, clientOptions: LanguageClientOptions, workspacePath: string) {
...
function openClientLogFile(logFile: string, column: ViewColumn = ViewColumn.Active): Thenable<boolean> {
	return new Promise((resolve) => {
		...
		glob(filename + '.*', { cwd: dirname }, (err, files) => {
			...
			openLogFile(logFile, 'Could not open CQL extension log file', column).then((result) => resolve(result));
		});
	});
}
