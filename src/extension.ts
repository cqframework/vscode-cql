...
import { getCqlConfiguration } from './utils';
			...
			return new Promise((resolve, reject) => {
				...
				const clientOptions: LanguageClientOptions = {
					...
					initializationOptions: {
						...
						settings: { java: getCqlConfiguration() },
				...
				if (!port) {
					...
					if (!lsPort) {
						serverOptions = prepareExecutable(requirements, workspacePath, getCqlConfiguration());
					} else {
						serverOptions = () => {
...
function setIncompleteClasspathSeverity(severity: string) {
	const config = getCqlConfiguration();
...
async function openFormatter(extensionPath) {
	...
	const formatterUrl: string = getCqlConfiguration().get('format.settings.url');
	...
	if (!fs.existsSync(file)) {
		...
		if (formatterUrl) {
			getCqlConfiguration().update('format.settings.url', (relativePath !== null ? relativePath : file), global);
	...
	await window.showInputBox(options).then(f => {
		if (f) {
			...
			if (isRemote(f)) {
				...
				getCqlConfiguration().update('format.settings.url', f, global);
			} else {
				...
				getCqlConfiguration().update('format.settings.url', (relativePath !== null ? relativePath : f), global);
				if (!fs.existsSync(f)) {
