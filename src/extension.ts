...
export class ClientErrorHandler implements ErrorHandler {
	...
	public error(_error: Error, _message: Message, count: number): ErrorAction {
		if (count && count <= 3) {
			logger.error(
				`${this.name} server encountered error: ${_message}, ${
		...
		logger.error(
			`${
				this.name
			} server encountered error and will shut down: ${_message}, ${
				_error && _error.toString()
			}`
		);
	...
	return requirements
		...
		.then(async (requirements) => {
			return new Promise<void>(async (resolve) => {
				...
				context.subscriptions.push(
					commands.registerCommand(Commands.OPEN_LOGS, () =>
						openLogs()
					)
				);
...
async function startServer(
	...
	await cqlLanguageClient.initialize(
		context,
		requirements,
		clientOptions,
		workspacePath
...
function openServerLogFile(
	...
	return openLogFile(
		serverLogFile,
		"Could not open CQL Language Server log file",
		column
	);
}
...
function openClientLogFile(
	...
	return new Promise((resolve) => {
		...
		glob(filename + ".*", { cwd: dirname }, (err, files) => {
			...
			openLogFile(
				logFile,
				"Could not open CQL extension log file",
				column
...
function openLogFile(
	...
	if (!fs.existsSync(logFile)) {
		return window
			.showWarningMessage("No log file available")
			.then(() => false);
	}
